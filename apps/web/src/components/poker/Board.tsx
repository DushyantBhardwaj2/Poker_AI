...
export default function Board() {
  const { potSize, currentStreet, boardCards, advanceStreet } = usePokerStore();

  const handleNextStreet = () => {
    const streets: typeof currentStreet[] = ['pre-flop', 'flop', 'turn', 'river', 'showdown'];
    const nextIdx = streets.indexOf(currentStreet) + 1;
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        ...
        <div className="flex gap-4 justify-center w-full mb-12">
          {displayCards.map((card, idx) => (
            ...
            >
              {card ? card : <span className="text-gray-800 text-xs font-mono">{idx + 1}</span>}
            </div>
          ))}
        </div>
